...
my $f=6/11;
my $eps=$ea*(zeroes(1,1,11)->zvals<5)+ $eb*(zeroes(1,1,11)->zvals>=5);
...
my $eps1l=($ea*(zeroes(11,1)->xvals<5)+ $eb*(zeroes(11,1)->xvals>=5))->slice("*1,*1")
   *identity(2);
my $g1l=Photonic::Geometry::FromEpsilonTensor
    ->new(epsilon=>$eps1l);
my $m1l=Photonic::WE::ST::Metric->new(
    geometry=>$g1l, epsilon=>pdl(1), wavenumber=>pdl(.0002),
    wavevector=>pdl([0,.000001])
    );
my $a1l=Photonic::WE::ST::Haydock->new(metric=>$m1l,
...
my $epst=($ea*(zeroes(11,1)->xvals<5)+ $eb*(zeroes(11,1)->xvals>=5))->slice("*1,*1")
         *identity(2);
my $gt=Photonic::Geometry::FromEpsilonTensor->new(epsilon=>$epst);
my $mt=Photonic::WE::ST::Metric->new(
    geometry=>$gt, epsilon=>pdl(1), wavenumber=>pdl(.0002),
    wavevector=>pdl([.000001,0])
    );
my $at=Photonic::WE::ST::Haydock->new(metric=>$mt,
   polarization=>pdl([0,1])->r2C, nh=>10, smallH=>1e-4);
$at->run;
my $ast=$at->as;
my $bst=$at->bs;
...
my $epss=r2C((1-$Bs)+5*$Bs)->slice("*1,*1")*identity(2);
my $gs=Photonic::Geometry::FromEpsilonTensor->new(epsilon=>$epss, L=>pdl(1,1));
my $ms=Photonic::WE::ST::Metric->new(geometry=>$gs, epsilon=>pdl(1),
   wavenumber=>pdl(.01), wavevector=>pdl([.001,0]));
my $als=Photonic::WE::ST::Haydock
...

{
    ...
    my $epse=r2C((1-$Be)+5*$Be)->slice("*1,*1")*identity(2);
    my $ge=Photonic::Geometry::FromEpsilonTensor->new(epsilon=>$epse, L=>pdl(1,1));
    my $me=Photonic::WE::ST::Metric->new(
	geometry=>$ge, epsilon=>pdl(1),	wavenumber=>pdl(.01),
	wavevector=>pdl([.001,0]));
    my $ale=Photonic::WE::ST::Haydock
