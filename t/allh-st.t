...
my $f=6/11;
my $eps=r2C($ea*(zeroes(1,1,11)->zvals<5)+ $eb*(zeroes(1,1,11)->zvals>=5));
my $g=Photonic::Geometry::FromEpsilonTensor
    ->new(epsilon=>$eps, Direction0=>pdl([1]));
my $a=Photonic::LE::ST::Haydock->new(geometry=>$g, nh=>10);
$a->run;
my $as=$a->as;
my $bs=$a->bs;
my $b2s=$a->b2s;
...
my $epst=($ea*identity(2)*(zeroes(2,2,11,1)->zvals<5)
         +$eb*identity(2)*(zeroes(2,2,11,1)->zvals>=5)
	 )->mv(2,3);
my $gt=Photonic::Geometry::FromEpsilonTensor
