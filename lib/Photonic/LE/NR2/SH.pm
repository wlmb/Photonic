...
sub _build_dipolar {
    ...
    #Fourier transform
    my $Esquare_G=fftn($Esquare_R, $ndims); # RorI nx ny...
    ...
    my $nablaE2=ifftn($iGE2->mv(1,-1), $ndims)->mv(-1,1); #RorI, cartesian, nx, ny...
...
sub _build_quadrupolar {
    ...
    #Fourier transform
    my $naaEE_G=fftn($naaEE_R->mv(1,-1)->mv(1,-1), # RorI cartesian cartesian nx ny...
			$ndims)->mv(-1,1)->mv(-1,1); # Get cartesians out of the way and thread
    my $G=$self->nrf->nr->G; #cartesian, nx, ny...
    my $iG=i2C $G; #RorI cartesian nx ny...
    my $iGnaaEE_G=($iG->(,,*1)*$naaEE_G)->sumover; #dot - RorI cartesian nx ny...
    $iGnaaEE_G = $iGnaaEE_G*$self->nrf->filter->(*1) if $self->nrf->has_filter; # nx ny...
    #back to real space. Get cartesian out of the way and then back
    my $P= ifftn($iGnaaEE_G->mv(1,-1), $ndims)->mv(-1,1); #RorI, cartesian, nx, ny...
    return $P;
}
