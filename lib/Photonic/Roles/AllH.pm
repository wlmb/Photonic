...
requires 'iterate', '_iterate_indeed'; #provided by OneH instance
...
sub _checkorthogonalize 
{
	...
	$next_W+=_sign($next_W)*2*$self->accuracy; #Epsilon?
	$next_W/=$self->next_b;
    }
	...
	($nextState,$currentState)=
	    map {$_-$s*HProd($s, $_)}($nextState, $currentState);
    }
    ...
    $nextState=$nextState-$currentState*HProd($currentState, $nextState);
